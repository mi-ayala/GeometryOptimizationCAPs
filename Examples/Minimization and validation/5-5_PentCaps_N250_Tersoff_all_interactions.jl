...
function Tersoff_zone_energy(X::AbstractMatrix{},
    p,
    zone::AbstractMatrix{UInt8},
    nbrs)
    @unpack N, a, b, λ₁, λ₂, β, n, h, c, d = p
    @assert size(X, 1) == N && size(X, 2) == 3
    @assert size(zone, 1) == N && size(zone, 2) == N

    T = eltype(X)          # Interval{Float64} (typically)
    oneT = one(T)
    zeroT = zero(T)

    # build integer constants using oneT (no literal 2, 0.5, etc.)
    twoT = oneT + oneT
    halfT = oneT / twoT

    # promote parameters to T (interval)
    aT = T(a)
    bT = T(b)
    λ1T = T(λ₁)
    λ2T = T(λ₂)
    ...
    @inbounds for i in 1:N
        ...
        for j in nbi
            wij = zone_weight(zone[i, j], oneT, zeroT)
            # if you *guarantee* zone 3 never appears in nbrs, you can omit this:
            # (wij == zeroT) && continue
            ...
            for k in nbi
                (k == j) && continue
                wik = zone_weight(zone[i, k], oneT, zeroT)
                ...
                # cos(angle) = proj; avoid acos (rigorous + avoids domain issues)
                proj = (dxik * dxij + dyik * dyij + dzik * dzij) / (rij * rik)

                # g(proj) = 1 + (c/d)^2 - c^2 / (d^2 + (h - proj)^2)
            ...
            e += wij * (aT * exp(-λ1T * rij) + bij * (-bT) * exp(-λ2T * rij))
        end
    end
...
@inline function zone_weight(z::UInt8, oneT, zeroT)
    # zone 1: definitely inside  -> weight = 1
    # zone 2: uncertain/transition -> weight ∈ [0,1] (rigorous, no cutoff eval)
    # zone 3: definitely outside -> weight = 0
    if z == UInt8(1)
        return oneT
    elseif z == UInt8(2)
        return interval(zeroT, oneT)  # [0,1] with matching endpoint type
    else
        return zeroT
    end
end
...
### Newton refinement
GF = x -> -ForwardDiff.gradient(e, x)
HF = x -> -ForwardDiff.hessian(e, x)
F_ext = x -> extended_Grad_Full_Tersoff(x, x_BFGS, GF)
DF_ext = x -> extended_Hess_Full_Tersoff(x, x_BFGS, GF, HF)
x_newton = newton_method(x -> (F_ext(x), DF_ext(x)), [zeros(6); reshape(x_BFGS, :, 1)]; tol=1.0e-11, maxiter=5)[1]
...
#  F_int(x_newton_interval)
# DF_int(x_newton_interval)

@time r = get_proof(x_newton, F_int, DF_int, 9.9212e-12)

# ┌ Info: success: the root and R delimit an interval of existence
# │ Y  = [0.0, 4.31699e-11]_com_NG
# │ Z₁ = [3.59434e-15, 0.000114436]_com_NG
# │ R  = 9.9212e-11
# └ root = [0.0, 4.31748e-11]_com_NG
# 3153.843118 seconds (165.69 M allocations: 541.223 GiB, 0.88% gc time, 0.19% compilation time: 7% of which was recompilation)
# [4.31747e-11, 9.92121e-11]_com_NG